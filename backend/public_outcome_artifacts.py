...
def _validation_summary(manifest: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    reported = report.get("reported_metrics") if isinstance(report.get("reported_metrics"), dict) else {}
    reported_primary = (
        reported.get("primary")
        if isinstance(reported.get("primary"), dict)
        else {}
    )
    ...
    return {
        ...
        "pr_auc": (
            ...
            else _safe_float(discr.get("wildfire_risk_score_pr_auc"))
        ),
        "brier_score": (
        ...
        "metrics_source": (reported.get("primary_source") or "in_sample_fallback"),
