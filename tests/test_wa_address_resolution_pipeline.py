...
def test_conflicting_high_confidence_candidates_return_ambiguous_conflict(monkeypatch):
    auth.API_KEYS = set()
    ...
    assert detail["resolution_status"] == "ambiguous_conflict"
    assert detail["needs_user_confirmation"] is True
    assert detail["rejection_category"] == "ambiguous_conflict"
