...
from backend.data_prep.catalog import ingest_catalog_raster
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest one canonical raster layer into data/catalog for reuse across many region builds."
    )
    parser.add_argument("--layer", required=True, help="Layer name (dem, fuel, canopy, etc.)")
    parser.add_argument("--source-path", default=None, help="Local source raster path")
    parser.add_argument("--source-url", default=None, help="Source URL for raster download")
    parser.add_argument("--source-endpoint", default=None, help="Provider endpoint (for bbox-first exports)")
    parser.add_argument("--provider-type", default="file_download", help="Provider type (arcgis_image_service, file_download, local_file)")
